...
def per_leg_p(a_rate, b_rate):
    """P1-ova vjerovatnoća da osvoji leg (log5 iz career leg-win%)."""
    den = a_rate * (1 - b_rate) + b_rate * (1 - a_rate)
    return a_rate * (1 - b_rate) / den if den > 0 else 0.5

def sweep_resistance(player_id, matches):
    """Koliko često igrač, kad izgubi (bo7), ipak osvoji bar 1 leg (nije pometen 0:4)."""
    losses = swept = gotleg = 0
    for m in matches:
        if max(m['p1legs'], m['p2legs']) != 4:
            continue
        if m['p1id'] == player_id:
            my, opp = m['p1legs'], m['p2legs']
        elif m['p2id'] == player_id:
            my, opp = m['p2legs'], m['p1legs']
        else:
            continue
        if my < opp:
            losses += 1
...
# ========== Over 5.5 Legs ==========
    ...
    with st.expander("💎 Value kalkulator"):
        ...
        # Live savjet: 2:0 zaostatak -> osvojiti leg
    with st.expander("💡 Savjet za 2:0 zaostatak (igrač osvoji leg)"):
        st.caption("Ako tokom live meča jedan igrač gubi 2:0, ovdje je preporuka da osvoji bar 1 leg "
                   "(neće biti pometen 0:4), potkrijepljena njegovom statistikom i modelom.")
        for pid, pname_x, opp_id in [(p1_id, p1_name, p2_id), (p2_id, p2_name, p1_id)]:
            L, sw, gl = sweep_resistance(pid, all_matches)
            px = per_leg_p(LEG_WR.get(pid, 0.5), LEG_WR.get(opp_id, 0.5))
            p_leg20 = 1 - (1 - px) ** 2          # P(osvoji bar 1 leg iz 2:0 zaostatka)
            fair = 1 / p_leg20 if p_leg20 > 0 else 0
            if L == 0:
                st.write(f"**{pname_x}**: nema dovoljno poraza za statistiku.")
                continue
            emp = gl / L * 100
            st.markdown(
                f"**Ako {pname_x} gubi 2:0 → igraj: {pname_x} osvaja leg**\n"
                f"- Statistika: u **{gl}/{L}** poraza ({emp:.0f}%) osvojio bar 1 leg "
                f"(pometen 0:4 samo {sw} puta).\n"
                f"- Model iz 2:0: **{p_leg20*100:.0f}%** → fer kvota **{fair:.2f}** "
                f"(kladi ako je live kvota iznad)."
            )
